...
if __name__ == '__main__':
    ...
    if not os.path.exists('history_sensitivity_analysis/1_layer'):
        os.makedirs('history_sensitivity_analysis/1_layer')
    
    if not os.path.isfile('history_sensitivity_analysis/1_layer/' + str(BATCH_SIZE) + args.replace('_result', '_injected_summary')):
        output_data.to_csv('history_sensitivity_analysis/1_layer/' + str(BATCH_SIZE) + args.replace('_result', '_injected_summary'), index = False, header = True)
    else: # else it exists so append without writing the header
        output_data.to_csv('history_sensitivity_analysis/1_layer/' + str(BATCH_SIZE) + args.replace('_result', '_injected_summary'),index = False, mode = 'a', header = False)
