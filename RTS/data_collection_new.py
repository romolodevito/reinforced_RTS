...
if __name__ == '__main__':
    ...
    for i in range(0,len(test_classes)-1):                    # for line in in_file:
        ...
        if xml_name:
            ...
            temp_data = pd.DataFrame({"test_class_name":[root.attrib.get('name').replace('.','/') + '.java'], "cycle_id":[commit_id],  "tests":[root.attrib.get('tests')], "failures":[root.attrib.get('failures')], "failures_%":[float(root.attrib.get('failures')) / float(root.attrib.get('tests'))], "time":[root.attrib.get('time')], "timestamp":[time.time()]})
    ...
    cycle_data = pd.DataFrame({"cycle_id":[commit_id], "time_end_end[sec]":[runtime], "selection_time":[selection_time]})
    ...
    if not os.path.isfile('times.csv'):
        cycle_data.to_csv('times.csv', index = False)
    else: # else it exists so append without writing the header
        cycle_data.to_csv('times.csv',index = False, mode = 'a', header=False)
